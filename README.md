...
4. NVIDIA–SK 等友岸事件與「兩隊」框架如何對讀，而不把假說寫成事實？

## Scope

### Included

- 梁文鋒外傳講話精校與心智模型拆解（**Lead／未驗證轉寫**）。
- 與融資、開源、API 定價、AGI 路線相關的公開報道核查。
- 二手評論（X / 財經自媒體）的事實層 vs 故事層分離。
- NVIDIA–SK 等公開合作事件與輿情**有界樣本**。
...
- 為 Hub 與其他產業專案提供可引用錨點——但錨點必須標清證據身分。
...
> **規則**：分析品質高，**不會**讓底層來源自動升級。文件角色與來源類型分開評。
...
- 官方技術文件證明生產路徑已全面脫離 CUDA——才可上調 TileLang 相關主張。

## Decision Log

| 日期 | 決定 | 理由 | 影響文件 |
|---|---|---|---|
| 2026-07-24 | 建立講話精校與深度拆解 | 外傳內容需結構化 | `01_speech.md`, `02_speech_deep_analysis.md` |
...
- `01`、`03`、`05_1` 含外傳長文／推文／討論串摘錄：研究必要範圍內保留；均須標 **原始 URL、作者、時間、擷取語境**。
- 引文／轉寫／自行分析邊界必須清楚；整理稿 **不得** 標成 Primary。
- 非必要全文以連結或允許的 archive 為優先。
